...
function parsePersistedInteraction(
  ...
  if (kind === "unsupported") {
    ...
    if (typeof metadata !== "object" || metadata === null || Array.isArray(metadata)) {
      return { kind: "unsupported", interactionId };
    }
    const sourceKind = (metadata as Record<string, unknown>).sourceKind;
    if (sourceKind !== null && (typeof sourceKind !== "string" || !/^[a-z_]{1,40}$/u.test(sourceKind))) {
      return null;
    }
...
export class ControllerInteractionRepository implements ControllerInteractionStore {
  ...
  /** The BB observation source may record only against the exact live turn and generation. */
  public sourceCanRecord(input: ControllerLeaseFence & {
    turnId: string;
    controllerKey: string;
    bbThreadId: string;
    controllerGenerationId: string;
    ...
    return turn !== undefined && turn.accepted_finalization_id === null;
  }
