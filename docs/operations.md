...
A schedule is a BB agent automation with a UTC cron, created only from a turn you sent yourself. An invalid cron expression is rejected before BB is asked. A schedule BB cannot read back exactly is not activated, and one whose project policy is later disabled is paused at the next reconciliation sweep rather than left running.
