...
`access list` returns only locally stored, secret-free binding metadata and never contacts the broker. `access status` runs the same diagnostic health check as the doctor and, when given a binding id, reports that one binding's state and generation. Neither command resolves, enrolls, or verifies a credential. Typed provider identity probes are fixed Convex/Vercel adapters (including the governed Vercel browser journey), and can only be requested by the owner from Telegram. Protected-host enrollment uses `connector binding enroll --stdin`, returns a secret-free projection for reconciliation, and creates no credentials or live provider resources. Current full readiness is required before and after dispatch; timeout, stale authority, mismatch, and ambiguity fail closed.
