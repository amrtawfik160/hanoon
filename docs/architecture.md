...
1. **Telegram I/O** polls updates, validates the paired private owner, records accepted input, and delivers drafts, status edits, callbacks, and final messages. Photos, image files, GIFs, short videos, and PDF, Markdown, or plain-text documents are accepted on the same owner turn. Stills attach as BB images; documents attach as BB files, and short text documents are also inlined into the dispatch. Messages sent in quick succession are claimed together as one burst, with each message's provenance kept. GIFs and videos are downloaded and sampled into an ordered set of stills the controller can see; if sampling is unavailable, Telegram's preview still is used instead.
...
The repository locks one reviewed 36-skill catalog. It combines 25 promoted Matt Pocock skills at revision `6654f6b60cd9d5be8b54c6fafe44346dabeb3b76` with eleven retained platform, guard, delivery, writing, visualization, and communication skills. Superpowers workflow ids, the duplicate discovery root, and the proportional workflow router are absent from the installed bundle. No runtime path downloads or repairs skills.
...
Each lock record binds the skill id, source path, source revision, source digest, descriptor digest, license, and invocation class. The 36-skill catalog allows model-invoked skills in general workers and requires an explicit navigator or owner route for user-invoked skills.
...
`npm run skills:verify` checks the package root order, schema 2 lock, the exact 36-skill catalog with empty legacy and shadow lists, bounded regular files, complete SHA-256 coverage, frontmatter identity, invocation metadata, nested local Markdown resources, and all source provenance and licenses. Build and activation run this verifier before registration. Drift, missing support files, a path escape, a symlink, malformed metadata, a leftover workflow or discovery kit, a duplicate catalog entry, or an admitted `do-*` id stops activation. The verifier never changes the bundle.
...
A monitor is a durable obligation, not a reminder. It watches a BB thread for completion or failure. Thread watches are event-driven: BB realtime and lifecycle events wake the executor instead of waiting for the next idle poll. Firing is claimed before it happens, so a crash mid-fire cannot double-book it, and a one-shot watch retires once it has fired. The agent receives its own instruction back as an ordinary turn, acts on it, and reports to the owner.

Clock schedules are not monitors. A schedule the agent sets becomes a BB agent automation with a UTC cron, bound to the controller's project and to the controller's own provider, model, and permission mode. BB owns the schedule and its run history; Hanoon owns a binding that records the automation id, the authority it was created under, and the exact definition BB read back. Creating a schedule persists BB's id before the read-back, adopts an already-existing automation under the same name rather than creating a second one, and activates the binding only when BB reads back the exact definition. A reconciliation sweep re-reads each binding about once a minute, records new runs as redacted evidence, pauses an agent schedule whose project policy or controller is no longer current, and cancels any leftover plugin-local clock row so one task can never fire through two schedulers. BB does not bound an agent run's wall-clock time or output size, so Hanoon classifies each finished run against the declared contract after the fact and reports a violation instead of claiming BB stopped it. Only a turn the owner sent can create or change a schedule; a scheduled or system turn cannot widen automations. BB refuses to host automations for some projects, and its personal project, where a single-owner controller runs, is one of them: there the schedule stays a plugin-local clock row that the monitor service fires, and the plugin says so rather than retrying BB. Schedules are for clock time, not for polling whether a thread or job has moved.
...
Several messages sent in quick succession are one request, not one request per message. Grouping happens at claim time, not at intake: the ingress keeps persisting each Telegram message the moment it arrives, and the controller claims a burst — the oldest queued owner turn plus every following queued owner turn whose receipt was within a two-second quiet gap of the one before it, in ordinal order. The claim waits out the quiet gap, so a burst still in flight is not cut and a lone message waits at most one gap. The caps are fixed constants: 25 messages per burst, 32 KB of rendered transcript, and 10 attachments; a burst that hits a cap closes there and the remainder forms the next burst. A system turn, a lifecycle follow-up, or an untranscribed voice note — which must keep its ordinal place — also closes a burst. A system turn or lifecycle follow-up never leads one either: it is claimed at once, on its own, and takes none of the owner's messages that arrived behind it.

The first turn of a burst is the leader and carries the dispatch. Every other member is settled in the same transaction as the claim with the folded outcome the steer path already uses, so each Telegram update is accounted for and the owner gets exactly one answer: the leader's answer is the burst's answer, and no separate acknowledgement is sent for it. The conversation digest records the burst once, as the leader's attributed transcript with that answer. Mid-answer arrivals fold differently only in what the owner sees: the waiting burst is selected under the same gap and caps, the steer is reserved for its leader — whole or not at all — and one combined addition reaches the running answer with one "got that" acknowledgement per folded burst. A burst whose leader fails before doing anything is replayed whole: its members follow the leader's replacement.

Each turn carries a structured provenance record — the message kind, the forwarded sender's name or a hidden-sender marker or a chat title, the quoted message's author and text when Telegram supplies them, the message id a reply refers to, and the album id — and the dispatch renders the burst as one attributed transcript with a numbered entry per member ("You:", "Forwarded from Tom Counsell:", "Reply to Tom Counsell's message above:", "File attached: …"). A lone message in the owner's own words renders exactly as before; a lone forward or reply keeps its attribution line. Whether the parts belong together is the model's judgment from that transcript, never a heuristic the plugin guesses; a conduct rule says one request until clearly not, and to name and answer clearly unrelated parts.

PDF, Markdown, and plain-text documents are accepted as attachments. A document turn keeps its caption as the message text ("Please read this file." when there is none), the file is downloaded through the existing Telegram download and uploaded as a BB file attachment with a 20 MB cap, and Markdown or plain-text files of up to 64,000 characters are additionally inlined into the dispatch text so a short brief is readable without a tool call. A text document is downloaded once per dispatch: the bytes fetched to inline it are the bytes uploaded. A document that cannot be prepared draws a failure notice that names the file types expected, distinct from the image notice. Inlined bodies exist only in the dispatch — never in the digest, the persisted transcript, or any evidence — so a forwarded document cannot leak through the plugin's records. An unsupported file type, or one over the cap, draws one plain reply at intake and queues nothing; several unsupported files arriving together draw one reply.
...
The plugin installs upkeep schedules it owns as BB agent automations, keyed so installation is idempotent across restarts and separate from the owner's own watches: the agent's upkeep must not crowd out schedules the owner set. They install on the first executor pass that finds a controller with a verified BB host and project, because pairing can happen long after the plugin starts. An installation that still carries the older plugin-local monitor rows hands each one over only after BB has read back an active schedule with a next run, then cancels the local row; a handover interrupted between those two steps is finished by the next reconciliation sweep.
