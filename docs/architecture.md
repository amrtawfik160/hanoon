...
The ladder is bounded at three attempts against the same block, counted per block rather than per job, so a job that clears one wall and stops at a later one arrives with a full allowance. When the ladder is spent the owner is told once, in plain language, and the job stops being re-decided. Merge and production promotion need authority the sweep cannot invent: for a project with no live grant it never drives past that boundary at all, and for a project that already merges unattended it may only re-fire that stage's own guarded effect, so the gates, the receipts, and the auto-approval decision all run again from scratch. It never drives past an unconfirmed cancellation or a project configuration gap it cannot fix.
