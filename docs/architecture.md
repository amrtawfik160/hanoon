...
Every worker stage receives a bounded structural map of the reference documents in scope. When a reference disagrees with the requested outcome or governing rule, the stage stops and asks the owner to resolve the specification conflict. Implementation-detail disagreements are reported without stopping the pipeline.
