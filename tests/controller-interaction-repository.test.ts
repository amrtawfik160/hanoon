...
it("pins the shipped migration bytes and appends the runtime repair migrations", () => {
  expect(ALL_MIGRATIONS).toHaveLength(
    80 + TICKET_41_MIGRATION_COUNT + NAVIGATOR_RELEASE_MIGRATIONS.length +
      ...
      MANAGED_AUTOMATION_STATE_UPGRADE_MIGRATIONS.length + NAVIGATOR_EFFECT_PROTOCOL_MIGRATIONS.length +
      NAVIGATOR_FINDING_LEDGER_UPGRADE_MIGRATIONS.length + NAVIGATOR_REVIEW_CONVERGENCE_UPGRADE_MIGRATIONS.length,
  );
