...
it("keeps every shipped migration at its original position and appends new ones", () => {
  expect(ALL_MIGRATIONS).toHaveLength(59);
  ...
  expect(ALL_MIGRATIONS[56]).toContain("controller_supervisor_steer_attempts_v2");
  expect(ALL_MIGRATIONS[57]).toContain("CREATE TABLE housekeeping_notices");
  expect(ALL_MIGRATIONS[58]).toContain("stall_notified_at");
