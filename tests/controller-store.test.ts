...
it("keeps controller and operation migrations before the appended pipeline migration", () => {
  ...
  expect(ALL_MIGRATIONS.at(-4)).toContain("CREATE TABLE controller_threads");
  expect(ALL_MIGRATIONS.at(-4)).toContain("CREATE TABLE controller_turns");
  expect(ALL_MIGRATIONS.at(-3)).toContain("dispatch_after_seq");
  expect(ALL_MIGRATIONS.at(-3)).toContain("telegram_message_id");
  expect(ALL_MIGRATIONS.at(-2)).toContain("CREATE TABLE thread_operations");
  expect(ALL_MIGRATIONS.at(-1)).toContain("CREATE TABLE pipeline_stage_attempts");
});
