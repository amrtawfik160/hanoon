import { CONTROLLER_METADATA_TOOL_NAMES } from "../src/controller/tools";
...
it("answers one owner turn end to end through the registered trust path", async () => {
  ...
  // The capability metadata tools register outside the domain manifest, so the
  // registered surface is the manifest followed by exactly those two.
  expect(fixture.harness.registrations.agentTools.map((tool) => tool.name)).toEqual([
