...
- **Only fixed operations exist end to end:** a diagnostic health check, a single-item vault resolve, and three typed provider identity probes (Convex, Vercel, and the fixed Vercel browser journey). There is no generic proxy, shell, URL fetch, or vault search/list reachable from Hanoon. Connector enrollment is only a protected-host `connector binding enroll --stdin` operation; Hanoon receives its secret-free projection, never target metadata or a credential. A live verification or identity probe can only be requested by the owner, in chat, through Hanoon's guarded tool, never from the plugin CLI.
