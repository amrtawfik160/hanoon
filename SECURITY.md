...
This foundation adds a second, deliberately separate trust boundary: a broker service meant to run on its own protected host or private service network, outside this plugin's process, database, and full-trust boundary. Nothing above in this document — "the plugin itself is trusted server-side code," the controller's shell and `bb` CLI reach, its default `full` permission mode — applies to the broker. The broker is designed so that a compromised controller or worker identity cannot reach its administrative interface, its filesystem, or the vault service-account token, and so that a compromised plugin never receives that token or a resolved credential value.
