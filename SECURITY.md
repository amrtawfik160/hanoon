...
- **Only fixed operation contracts exist end to end:** a diagnostic health check, a single-item vault-reference resolution contract, and typed Convex/Vercel provider identity probes. The governed Vercel browser operation is fail-closed in the default server composition. There is no generic proxy, shell, URL fetch, or vault search/list reachable from Hanoon. Connector enrollment is only a protected-host `connector binding enroll --stdin` operation; `access reconcile` is a mutating local Hanoon projection import through an authenticated controller mutation, never a broker call, target-metadata import, or credential import. A live verification or identity probe can only be requested by the owner, in chat, through Hanoon's guarded tool, never from the plugin CLI.
- **Provider calls are bounded and replay-safe.** The fixed TLS adapters terminate at the configured provider timeout or the persisted request deadline, reject redirects and non-authoritative response shapes, and return no provider body. Hanoon checks current full broker readiness before and after dispatch; an ambiguous retry reuses the exact persisted request envelope and receipt identity. The installed `@1password/sdk` 0.5.0 provides no `AbortSignal` for `Secrets.resolveAll` and no public client close/dispose operation, so SDK-backed credential resolution fails closed before invoking that operation. The injected abortable-port test is not evidence that the installed SDK transport can be cancelled.
- **The repository's acceptance proof is synthetic.** Controlled local vault/TLS fixtures cover the production composition and canary scans; they do not constitute evidence of access to a live provider or credential.
